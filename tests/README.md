...
Current: **PASS 141, FAIL 0, XFAIL 19, XPASS 0** (160 probes) — 2026-08-29, after the
measure-numbering enum fix (v1.4.5) and the OVERRIDES cleanup: 12 stale xfail rows for
already-fixed bugs are now EXTRA_KEYS baseline guards, so `gen_expected.py` and
`expected.json` are finally coherent (see `FIXLOG.md`).
The runner's failure path is proven: poisoning an expectation produces FAIL + exit 1.
`ABC2XML_BIN=<other-abc2xml.py>` runs the same expectations against a different
converter build — point it at the pre-fix copy and every fixed probe must go red.
...
## Fixed (kept for the record — ids are never reused)

- ~~**CONF-dynamics-directions-tempo-1**~~ **FIXED 2026-08-08** — `Q:1/4=3/8` now emits the metric-modulation beat-unit pair and a relative `<sound tempo>`.
- ~~**CONF-abcplus-extensions-1**~~ **FIXED 2026-08-08** — `%%mute` now reaches `<sound mute="yes">`; `!+!`/`!plus!` hand stop unchanged.
- ~~**CONF-abcplus-extensions-2**~~ **FIXED 2026-08-08** — `%%midi-bank/vol/pan` travel with their voice out of the header; `%%MIDI bank N` has a regex.
- ~~**CONF-structure-repeats-4**~~ **FIXED 2026-08-08** — quoted volta text keeps its `-`, and a text-only ending carries the EMPTY ending-number (the `33` sentinel is gone).
- ~~**CONF-ornaments-3**~~ **FIXED 2026-08-08** — single-note tremolo covers the full `tremolo-marks` range 1-8, both slash and `trem<N>` forms.
- ~~**CONF-structure-repeats-3** (follow-up)~~ **FIXED 2026-08-08** — `%%tocoda NAME` emits `<sound tocoda="NAME">` (the jump target), not `coda=` (the coda label).
