...
ROOT = TESTS.parent
# ABC2XML_BIN lets the SAME expectations be run against a DIFFERENT converter build.
# That is how a fix is proven falsifiable: point it at the pre-fix copy and the probe
# must FAIL (exit 1); unset, it must PASS.  Default is unchanged (the canonical path).
ABC2XML = Path(os.environ.get("ABC2XML_BIN") or (ROOT / "abc2xml" / "abc2xml.py"))
PROBES = TESTS / "probes"
